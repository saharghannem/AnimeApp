...
class GeminiService
{
     ...
     * @param string $animeName Le nom de l'anime
     * @return array Les informations du trailer (url, titre, description)
     */
    public function findAnimeTrailer(string $animeName): array
    {
        // Base de données locale de trailers populaires
        $trailerDatabase = [
            'naruto' => [
                'url' => 'https://www.youtube.com/embed/QczGoCmX-pI',
                'title' => 'Naruto - Bande Annonce Officielle',
                'description' => 'Trailer officiel de Naruto Shippuden.'
            ],
            'one piece' => [
                'url' => 'https://www.youtube.com/embed/S8_YwFLCh4U',
                'title' => 'One Piece - Trailer Officiel',
                'description' => 'Bande-annonce de la série One Piece.'
            ],
            'attack on titan' => [
                'url' => 'https://www.youtube.com/embed/MGRm4IzK1SQ',
                'title' => 'L\'Attaque des Titans - Trailer Officiel',
                'description' => 'Trailer officiel de Shingeki no Kyojin (L\'Attaque des Titans).'
            ],
            'demon slayer' => [
                'url' => 'https://www.youtube.com/embed/VQGCKyvzIM4',
                'title' => 'Demon Slayer - Bande Annonce',
                'description' => 'Trailer officiel de Kimetsu no Yaiba (Demon Slayer).'
            ],
            'my hero academia' => [
                'url' => 'https://www.youtube.com/embed/EPVkcwyLQQ8',
                'title' => 'My Hero Academia - Trailer',
                'description' => 'Bande-annonce de Boku no Hero Academia.'
            ],
            'sword art online' => [
                'url' => 'https://www.youtube.com/embed/6ohYYtxfDCg',
                'title' => 'Sword Art Online - Trailer',
                'description' => 'Trailer officiel de la série Sword Art Online.'
            ],
            'death note' => [
                'url' => 'https://www.youtube.com/embed/NlJZ-YgAt-c',
                'title' => 'Death Note - Bande Annonce',
                'description' => 'Trailer officiel de Death Note.'
            ],
            'tokyo ghoul' => [
                'url' => 'https://www.youtube.com/embed/vGuQeQsoRgU',
                'title' => 'Tokyo Ghoul - Trailer Officiel',
                'description' => 'Bande-annonce de Tokyo Ghoul.'
            ],
            'hunter x hunter' => [
                'url' => 'https://www.youtube.com/embed/d6kBeJjTGnY',
                'title' => 'Hunter X Hunter - Trailer',
                'description' => 'Trailer officiel de Hunter X Hunter.'
            ],
            'fullmetal alchemist' => [
                'url' => 'https://www.youtube.com/embed/--IcmZkvL0Q',
                'title' => 'Fullmetal Alchemist: Brotherhood - Trailer',
                'description' => 'Bande-annonce de Fullmetal Alchemist: Brotherhood.'
            ],
            'dragon ball' => [
                'url' => 'https://www.youtube.com/embed/FHgm89hKpXU',
                'title' => 'Dragon Ball Super - Trailer',
                'description' => 'Trailer officiel de Dragon Ball Super.'
            ],
            'steins gate' => [
                'url' => 'https://www.youtube.com/embed/uMYhjVwp0Fk',
                'title' => 'Steins;Gate - Bande Annonce',
                'description' => 'Trailer officiel de Steins;Gate.'
            ],
        ];
        
        // Recherche dans la base de données locale (insensible à la casse)
        $animeLower = strtolower(trim($animeName));
        
        // Recherche directe
        if (isset($trailerDatabase[$animeLower])) {
            return $trailerDatabase[$animeLower];
        }
        
        // Recherche partielle
        foreach ($trailerDatabase as $key => $trailer) {
            if (strpos($animeLower, $key) !== false || strpos($key, $animeLower) !== false) {
                return $trailer;
            }
        }
        
        // Si aucun trailer n'est trouvé, renvoyer un trailer générique d'anime
        return [
            'url' => 'https://www.youtube.com/embed/CmTeYj2FmRc',
            'title' => 'Trailer générique pour ' . $animeName,
            'description' => 'Aucun trailer spécifique trouvé pour cet anime.',
            'is_external' => false
        ];
    }
    
    /**
     * Méthode privée pour générer du contenu avec l'API Gemini
     */
    /**
     * Construit le prompt pour l'extraction de trailer avec Gemini
     */
    private function buildTrailerPrompt(string $animeName, array $searchResults): string
    {
        $searchResultsText = '';
        foreach ($searchResults as $index => $result) {
            $searchResultsText .= "\n{$index}. Titre: {$result['title']}\nLien: {$result['link']}\nDescription: {$result['snippet']}\nSource: {$result['source']}";
        }
        
        return <<<EOT
Analyse ces résultats de recherche pour le trailer de l'anime "{$animeName}" et détermine le lien qui est le plus susceptible d'être un trailer officiel.

IMDB est la source prioritaire si un résultat d'IMDB est disponible. Sinon, cherche un lien YouTube officiel de bonne qualité.

Résultats de recherche:{$searchResultsText}

Extrait uniquement l'URL que tu considères comme étant le meilleur trailer officiel. Si plusieurs sont valides, prends le plus récent et de la meilleure qualité.
Le format de ta réponse doit être exactement:
{"url":"[L'URL COMPLÈTE DU TRAILER]","title":"[TITRE DESCRIPTIF]","description":"[COURTE DESCRIPTION]"}

N'inclus aucun autre texte explicatif dans ta réponse, seulement ce JSON.
EOT;
    }
    
    /**
     * Analyse la réponse de Gemini pour extraire les infos du trailer
     */
    private function parseGeminiTrailerResponse(string $response, array $searchResults): array
    {
        // Nettoyer la réponse
        ...
        if (preg_match('/{.*}/s', $response, $matches)) {
            ...
            if (json_last_error() === JSON_ERROR_NONE && isset($data['url'])) {
                // Formater l'URL YouTube si nécessaire
                $data['url'] = $this->formatYoutubeUrl($data['url']);
                return $data;
            }
        }
        
        // Si on ne peut pas extraire de JSON, chercher une URL YouTube dans la réponse
        if (preg_match('/https?:\/\/(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]{11})/', $response, $matches)) {
            $youtubeUrl = $matches[0];
            return [
                'url' => $this->formatYoutubeUrl($youtubeUrl),
                'title' => 'Trailer de ' . $animeName,
                'description' => 'Bande-annonce trouvée par recherche automatique'
            ];
        }
        
        // Si aucune URL n'est trouvée, retourner un tableau vide
        return [];
    }
    
    /**
     * Formate une URL de trailer pour l'intégration
     */
    private function formatYoutubeUrl(string $url): string
    {
        // Traiter les URLs IMDB
        if (preg_match('/imdb\.com\/video\/vi([0-9]+)/i', $url, $matches) || 
            preg_match('/imdb\.com\/title\/tt[0-9]+\/videoplayer\/vi([0-9]+)/i', $url, $matches)) {
            $videoId = $matches[1];
            return 'https://www.imdb.com/video/imdb/vi' . $videoId . '/imdb/embed';
        }
        
        // Traiter les URLs IMDB Trailers
        if (strpos($url, 'imdb.com/trailers') !== false) {
            // Pour les pages de collection de trailers, nous retournons simplement l'URL
            // car elles ne sont pas directement intégrables
            return $url;
        }
        
        // Convertir les liens youtu.be en youtube.com
        if (preg_match('/youtu\.be\/([a-zA-Z0-9_-]{11})/', $url, $matches)) {
            return 'https://www.youtube.com/embed/' . $matches[1];
        }
        
        // Convertir les liens youtube.com/watch en format embed
        if (preg_match('/youtube\.com\/watch\?v=([a-zA-Z0-9_-]{11})/', $url, $matches)) {
            return 'https://www.youtube.com/embed/' . $matches[1];
        }
        
        // Si c'est déjà un lien embed ou un autre type de lien, le retourner tel quel
        return $url;
    }
    
    /**
     * Retourne un trailer par défaut en cas d'échec
     */
    private function getFallbackTrailer(string $animeName): array
    {
        return [
            'url' => '',
            'title' => 'Trailer non disponible pour ' . $animeName,
            'description' => 'Aucun trailer n\'a pu être trouvé automatiquement. Veuillez rechercher manuellement.'
        ];
    }
    
    private function callGeminiAPI(string $userMessage, array $animeData)
    {
...
PROMPT;
    ...
    /**
     * Génère du contenu directement avec l'API Gemini
     */
    public function generateContentDirect(string $prompt)
    {
        try {
            // Appeler l'API Gemini avec un prompt simple
            $response = $this->httpClient->request('POST', 'https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=' . $this->apiKey, [
                'timeout' => 8, // Timeout de 8 secondes
                'max_duration' => 15, // Durée maximale totale de 15 secondes
                'json' => [
                    'contents' => [
                        [
                            'parts' => [
